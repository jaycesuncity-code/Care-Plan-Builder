...
//   - `includedIn` add-ons are $0 for the plans listed (Premier)
...
export const ADDON_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "wss", name: "Water Softener Salt", price: 68, includedIn: ["premier"] },
      { id: "wsv", name: "Water Softener Service", price: 75 },
      { id: "ros", name: "Reverse Osmosis Service", price: 50, includedIn: ["premier"] },
 ...
 * taking the highest quantity — the builder can emit the same add-on twice
 * for a Premier included add-on that was selected under a previous plan, and
 * that must never bill twice.
 *
...
export function priceSelection(planId, requestedAddons) {
  ...
  for (const group of ADDON_GROUPS) {
    for (const base of group.items) {
      ...
      if (covered && isIncludedFree) {
        // Always recorded as a $0 included line while the plan covers it,
        // whether or not the client sent it — mirrors the builder's third
        // pass in renderTotal(), and closes the double-charge case where a
        // stale selection from a previous plan is still in the payload.
        lines.push(makeLine(item, 1, 0, true, false, false));
        continue;
      }
